...
Pero... ya mencionaste arriba que "tiene similitudes con Java (y por lo tanto, algo de C/C++)". Pensé que esto se trataba de "sus propias características"
...
Me parece que los que hace Java se menciona demasiado. ¿Que pasa si no soy programador de Java?
Cada caracter en una cadena está en formato Unicode y está representado por un número de 16 bits.
Too much information y muy poca al mismo tiempo. Si no se nada de JS, como me va a importar si son 16, 24 o 32 bits? Si el ejemplo tuviera acentos IMHO ayudaría a aclarar lo único que (como aprendiz de JS) me interesa: si puedo usarlos sin necesidad de una sintáxis especial (mas tarde habrá tiempo para chocar con la dura realidad)
...
¿Ambitos? ¿Ya se ha hablado de ámbitos? ¿Viene en el examen? Esto debería estar en la sección de funciones, en vez de aquí (es más claro después de que expliques que las funciones crean ámbitos)
...
### Arrays
Los arreglos (*arrays*) son un tipo especial de objetos y representan colecciones que pueden guardar cualquier tipo de dato en JavaScript y sus elementos pueden o no ser del mismo tipo, a diferencia de otros lenguajes como Java que solo permiten guardar un solo tipo de datos por arreglo. De nuevo con Java... si quien lee conoce Java, esa comparación la hará mentalmente; si no, es lo mismo que le hablaras de como se manejan los arrays en brainfuck.

Los arreglos tienen una propiedad especial ¿más que otras? llamada `length` y utilizan los corchetes (`[]`) para acceder a un elemento del arreglo a través de su índice. En JavaScript, el índice de los arreglos empieza en 0 y el valor de `length` es igual al último índice del arreglo más uno.
...
#### `Object.create`
Esta es una tercera forma de crear un objeto, permitiendo además definir sus propiedades y cuál será su *prototype*. A esta altura aún no se que es un prototype. Quizas podrías agregar una breve nota
...
### Funciones
Las funciones en JavaScript permiten crear operaciones reutilizables. Estas tienen acceso a sus argumentos mediante la palabra reservada `arguments`, la cual es un objeto similar a un arreglo (tiene una propiedad `length` y los nombres de sus propiedades son índices que van desde 0 hasta `length`). Mencionado tan pronto, da la impresión de que fuera la forma principal de obtener los argumentos dentro de una función. Quizas podrías mover esta parte para después del ejemplo.
...
En las estructuras condicionales se utilizan los operadores de comparación, como `==`, `!=`, `===` y `!==`. La diferencia entre `==` y `===` es que el primero compara valores, mientras que el segundo compara valores y tipos de datos. De igual forma pasa con `!=` y `!==`. Técnicamente cierto, pero algo confuso si no te acuerdas de la conversión implícita que mencionaste en "Booleanos"
...
No vendria mal aquí una advertencia sobre que al iterar un objeto  tambien se iteran sobre propiedades heredadas (que posiblemente no es lo que quieres, etc).
